...
public class AcaciaHangingSignRenderer extends TileEntitySpecialRenderer<SignTileEntity>
{
    private static final ResourceLocation TEXTURE_ACACIA_HANGING_SIGN = new ResourceLocation(DATA.MODID + ":" +"textures/sign/acacia_hanging_sign.png");
    ...
    public void render(SignTileEntity tileEntity, double x, double y, double z, float partialTicks, int destroyStage, float alpha)
    {
        ...
        if (block == BlockRegister.get("acacia_hanging_sign_top_full"))
        {
            GlStateManager.translate((float)x + 0.5F, (float)y + 0.5F, (float)z + 0.5F);
            float f1 = (float)(tileEntity.getBlockMetadata() * 360) / 16.0F;
            GlStateManager.rotate(-f1, 0.0F, 1.0F, 0.0F);
        }
        else // Block == BlockRegister.get("acacia_hanging_sign_side")
        {
        ...
        {
            this.bindTexture(TEXTURE_ACACIA_HANGING_SIGN);
        }
        if (block == BlockRegister.get("acacia_hanging_sign_top_full"))
        {
