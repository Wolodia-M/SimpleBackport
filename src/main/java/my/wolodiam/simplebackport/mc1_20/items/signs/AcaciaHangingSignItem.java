...
public class AcaciaHangingSignItem extends Item {
    public AcaciaHangingSignItem(String name) {
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos blockPos, EnumHand hand, EnumFacing facing, float float1, float float2, float float3) {
        ...
        if (true) {
            ...
            if (player.canPlayerEdit(blockPos, facing, itemStack) && BlockRegister.get("acacia_hanging_sign_top_full").canPlaceBlockAt(world, blockPos)) {
                if (world.isRemote) {
                    ...
                    if (facing == EnumFacing.DOWN) {
                        ...
                        world.setBlockState(blockPos, BlockRegister.get("acacia_hanging_sign_top_full").getDefaultState().withProperty(BirchHangingSignTopFullBlock.ROTATION, pos1), 11);
                    } else {
                        world.setBlockState(blockPos,BlockRegister.get("acacia_hanging_sign_side").getDefaultState().withProperty(BirchHangingSignSideBlock.FACING, facing), 11);
                    }
