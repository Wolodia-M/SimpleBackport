...
import my.wolodiam.simplebackport.mc1_14.blocks.signs.OakSignSide;
import my.wolodiam.simplebackport.mc1_14.blocks.signs.OakSignTop;

public class OakSignItem extends Item {
    public OakSignItem(String name) {
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos blockPos, EnumHand hand, EnumFacing facing, float float1, float float2, float float3) {
        ...
        if (true) {
            ...
            if (player.canPlayerEdit(blockPos, facing, itemStack) && BlockRegister.get("oak_sign_top").canPlaceBlockAt(world, blockPos)) {
                if (world.isRemote) {
                    ...
                    if (facing == EnumFacing.UP) {
                        ...
                        world.setBlockState(blockPos, BlockRegister.get("oak_sign_top").getDefaultState().withProperty(OakSignTop.ROTATION, pos1), 11);
                    } else {
                        world.setBlockState(blockPos,BlockRegister.get("oak_sign_side").getDefaultState().withProperty(OakSignSide.FACING, facing), 11);
                    }
