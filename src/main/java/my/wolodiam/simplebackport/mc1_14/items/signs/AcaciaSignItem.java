...
import my.wolodiam.simplebackport.mc1_14.blocks.signs.AcaciaSignSide;
import my.wolodiam.simplebackport.mc1_14.blocks.signs.AcaciaSignTop;

public class AcaciaSignItem extends Item {
    public AcaciaSignItem(String name) {
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos blockPos, EnumHand hand, EnumFacing facing, float float1, float float2, float float3) {
        ...
        if (true) {
            ...
            if (player.canPlayerEdit(blockPos, facing, itemStack) && BlockRegister.get("acacia_sign_top").canPlaceBlockAt(world, blockPos)) {
                if (world.isRemote) {
                    ...
                    if (facing == EnumFacing.UP) {
                        ...
                        world.setBlockState(blockPos, BlockRegister.get("acacia_sign_top").getDefaultState().withProperty(AcaciaSignTop.ROTATION, pos1), 11);
                    } else {
                        world.setBlockState(blockPos,BlockRegister.get("acacia_sign_side").getDefaultState().withProperty(AcaciaSignSide.FACING, facing), 11);
                    }
