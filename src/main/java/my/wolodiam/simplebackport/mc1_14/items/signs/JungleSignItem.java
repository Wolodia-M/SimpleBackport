...
import my.wolodiam.simplebackport.mc1_14.blocks.signs.JungleSignSide;
import my.wolodiam.simplebackport.mc1_14.blocks.signs.JungleSignTop;

public class JungleSignItem extends Item {
    public JungleSignItem(String name) {
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos blockPos, EnumHand hand, EnumFacing facing, float float1, float float2, float float3) {
        ...
        if (true) {
            ...
            if (player.canPlayerEdit(blockPos, facing, itemStack) && BlockRegister.get("jungle_sign_top").canPlaceBlockAt(world, blockPos)) {
                if (world.isRemote) {
                    ...
                    if (facing == EnumFacing.UP) {
                        ...
                        world.setBlockState(blockPos, BlockRegister.get("jungle_sign_top").getDefaultState().withProperty(JungleSignTop.ROTATION, pos1), 11);
                    } else {
                        world.setBlockState(blockPos,BlockRegister.get("jungle_sign_side").getDefaultState().withProperty(JungleSignSide.FACING, facing), 11);
                    }
