...
import my.wolodiam.simplebackport.mc1_14.blocks.signs.BirchSignSide;
import my.wolodiam.simplebackport.mc1_14.blocks.signs.BirchSignTop;

public class BirchSignItem extends Item {
    public BirchSignItem(String name) {
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos blockPos, EnumHand hand, EnumFacing facing, float float1, float float2, float float3) {
        ...
        if (true) {
            ...
            if (player.canPlayerEdit(blockPos, facing, itemStack) && BlockRegister.get("birch_sign_top").canPlaceBlockAt(world, blockPos)) {
                if (world.isRemote) {
                    ...
                    if (facing == EnumFacing.UP) {
                        ...
                        world.setBlockState(blockPos, BlockRegister.get("birch_sign_top").getDefaultState().withProperty(BirchSignTop.ROTATION, pos1), 11);
                    } else {
                        world.setBlockState(blockPos,BlockRegister.get("birch_sign_side").getDefaultState().withProperty(BirchSignSide.FACING, facing), 11);
                    }
