...
import my.wolodiam.simplebackport.mc1_14.blocks.signs.DarkOakSignSide;
import my.wolodiam.simplebackport.mc1_14.blocks.signs.DarkOakSignTop;

public class DarkOakSignItem extends Item {
    public DarkOakSignItem(String name) {
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos blockPos, EnumHand hand, EnumFacing facing, float float1, float float2, float float3) {
        ...
        if (true) {
            ...
            if (player.canPlayerEdit(blockPos, facing, itemStack) && BlockRegister.get("dark_oak_sign_top").canPlaceBlockAt(world, blockPos)) {
                if (world.isRemote) {
                    ...
                    if (facing == EnumFacing.UP) {
                        ...
                        world.setBlockState(blockPos, BlockRegister.get("dark_oak_sign_top").getDefaultState().withProperty(DarkOakSignTop.ROTATION, pos1), 11);
                    } else {
                        world.setBlockState(blockPos,BlockRegister.get("dark_oak_sign_side").getDefaultState().withProperty(DarkOakSignSide.FACING, facing), 11);
                    }
